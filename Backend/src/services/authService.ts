...
export class authService {
  ...
  async registrarUsuarioOAuth(datos: {
    ...
    // Crear usuario OAuth inactivo hasta verificación
    const nuevoUsuario = await this.usuarioSrv.crearUsuario({
      nombre: datos.nombre,
      apellido: datos.apellido,
      correo: datos.correo,
      ...
      google_id: datos.proveedor === 'google' ? datos.google_id : null,
      microsoft_id: datos.proveedor === 'microsoft' ? datos.microsoft_id : null,
      activo: false // OAuth users also need verification
    });

    // Generar código de verificación para usuarios OAuth también
    const codigoEnviado = await this.generarYEnviarCodigo(nuevoUsuario.id_usuario, datos.correo);

    return { usuario: nuevoUsuario, codigoEnviado };
  }

  async generarYEnviarCodigo(usuarioId: number, email: string): Promise<boolean> {
