...
export class usuarioRepository {
  ...
  async findByEmail(email: string): Promise<usuario[]> {
    ...
    async create(data: Omit<usuario, 'id_usuario' | 'creado_at'>): Promise<usuario> {
        // Para usuarios OAuth, usar un hash especial si no hay password
        const passwordHash = data.password_hash || (data.google_id || data.microsoft_id ? 'oauth_user' : null);
        
        const query = `INSERT INTO usuarios
                       (descripcion, nombre, apellido, tipo, correo, activo, password_hash, google_id, microsoft_id, avatar_url)
